...
function buildHtml(data: ReportData): string {
  ...
  <html dir="rtl" lang="ar">
    ...
    <body>
      ...
      <div class="subtitle">
        ${escapeHtml(data.periodLabel)} (${escapeHtml(data.startDate)} - ${escapeHtml(data.endDate)})
        &nbsp;|&nbsp; ${escapeHtml(data.branchLabel)}
        &nbsp;|&nbsp; تاريخ التصدير: ${escapeHtml(generatedAt)}
      </div>

      <div class="summary">
        <div class="card"><div class="label">الدخل</div><div class="value positive">${escapeHtml(
          formatAmount(data.totalRevenue)
        )}</div></div>
        <div class="card"><div class="label">المصروف</div><div class="value negative">${escapeHtml(
          formatAmount(data.totalExpense)
        )}</div></div>
        <div class="card"><div class="label">صافي الربح</div><div class="value ${
          data.netProfit >= 0 ? 'positive' : 'negative'
        }">${escapeHtml(formatAmount(data.netProfit))}</div></div>
      </div>
      ...
      ${
        data.branchTotals.length > 1
          ? `<h2>مقارنة الفروع</h2>${tableOrEmpty(
              ['الفرع', 'الدخل', 'المصروف', 'صافي الربح'],
              branchRows,
              'لا توجد بيانات فروع.'
            )}`
          : ''
      ...
      <div class="footer">بيتزا الفرن - تقرير محاسبي من النظام</div>
    </body>
  </html>`;
}
...
function printHtmlOnWeb(html: string): void {
  ...
  if (!printWindow) {
    throw new Error('اسمح بفتح النوافذ المنبثقة ثم أعد محاولة تصدير التقرير.');
  }
